...
from app.test.audio import generate_local_fake_audio, TMP_CACHE_DIR
...
async def generate_audio(payload: GenerateAudioRequest, request: Request) -> GenerateAudioResponse:
    try:
        ...
        if MOCKED_RESPONSE:
            ...
            download_url = str(request.url_for("download_audio", audio_id=str(audio_id)))
        ...
        presigned_url = s3.upload_object(data=audio_buffer, key=str(audio_id) + ".wav", content_type="audio/wav",
            expires_in=600)
