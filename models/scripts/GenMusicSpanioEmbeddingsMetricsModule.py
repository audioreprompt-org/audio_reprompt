...
def compute_clap_scores(
    ...
    """
    Calcula el CLAP Score (similaridad texto-audio) usando el sistema modular de métricas.
    """
    set_reproducibility(42)
    device = device or ("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\nUsando dispositivo: {device}\n")

    # 1. Crear lista de CLAPItem desde los resultados generados
    items = [
        CLAPItem(
            id=r.id,
    ...
    print(f"Calculando CLAP Scores con backend '{backend}'...\n")

    # 2. Calcular los scores usando el sistema modular
    scored_items = calculate_scores(items, device=device, backend=backend)

    scored: list[MusicGenCLAPResult] = []
    embedding_records = []

    # 3. Construir resultados y guardar si es necesario
    for s in tqdm(scored_items, desc="Procesando resultados", ncols=80):
        clap_score = round(float(s.clap_score), 6)
        r = next((x for x in results if x.id == s.item.id), None)
        if not r:
            continue
        ...
        scored.append(
            MusicGenCLAPResult(
                ...
                instrument=s.item.instrument,
                audio_path=s.item.audio_path,
                clap_score=clap_score,
            )
        )

        if save_embeddings:
            # NOTA: Los backends no exponen embeddings explícitamente
            # así que aquí solo guardamos el score y metadatos
    ...
    if save_embeddings and embedding_records:
        ...
        print(f"\nCLAP Scores guardados en: {embeddings_csv_path}\n")
